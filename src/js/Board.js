  ...
  componentDidMount() {
    Dragula([this.swimlanes.backlog.current, this.swimlanes.inProgress.current, this.swimlanes.complete.current])
      .on('drop', (element, target) => {
        const cardType = target.classList[1]; // either 'Backlog', 'In', or 'Complete'
        let className;
        switch(cardType) {
          case 'Backlog': className = 'Card-grey'; break;
          case 'In': className = 'Card-blue'; break;
          case 'Complete': className = 'Card-green'; break;
          default: className = '';
        }
        element.className = 'Card ' + className;  // set class of card to match column for CSS color
      });
  }
